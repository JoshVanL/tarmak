...
// test the happy path, local key matches the one existing in EKS
func TestEKS_validateEKSKeyPairExistingHappyPath(t *testing.T) {
	a := newFakeEKS(t)
	defer a.ctrl.Finish()
	...
	err = a.EKS.validateAWSKeyPair()
	if err != nil {
		t.Errorf("unexpected error: %s", err)
	}
}

func TestEKS_validateEKSKeyPairExistingMismatch(t *testing.T) {
	a := newFakeEKS(t)
	defer a.ctrl.Finish()
...
func TestEKS_validateEKSKeyPairNotExisting(t *testing.T) {
	a := newFakeEKS(t)
	defer a.ctrl.Finish()
